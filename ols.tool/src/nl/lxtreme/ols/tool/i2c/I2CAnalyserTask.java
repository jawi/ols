 ...
 */
public class I2CAnalyserTask implements Callable<Void>
{
  // CONSTANTS

  public static final String EVENT_ACK = "ACK";
  public static final String EVENT_BUS_ERROR = "BUS-ERROR";
  ...
  private final boolean detectSDA_SCL;
   ...
   */
  public I2CAnalyserTask( final ToolContext aContext, final Configuration aConfiguration )
  {
    ...
    I2CConfig config = Configurable.createConfigurable( I2CConfig.class, aConfiguration.asMap() );

    // the default behaviour is remained as-is...
    this.detectSDA_SCL = config.detectLines();

    this.lineAidx = config.lineAIdx();
    this.lineAmask = ( 1 << this.lineAidx );

    this.lineBidx = config.lineBIdx();
    this.lineBmask = ( 1 << this.lineBidx );

    this.reportACK = config.reportAck();
    this.reportNACK = config.reportNack();
    this.reportStart = config.reportStart();
    this.reportStop = config.reportStop();
  }
  ...
  public Void call() throws ToolException
  {
    ...
    final int sdaMask = ( int )( 1L << this.sdaIdx );
    final int sclMask = ( int )( 1L << this.sclIdx );
        ...
        if ( bitCount == 0 )
        {
          ...
          if ( startCondFound )
          {
            ...
            if ( ( byteValue & 0xf8 ) == 0xf0 )
            {
              ...
              desc = String.format( "Setup %s 10-bit slave", ( direction == 1 ) ? "read from" : "write to" );
            }
            else
            {
              if ( tenBitAddress )
              ...
              desc = String.format( tenBitAddress ? "Setup %s slave: 0x%X " : "Setup %s slave: 0x%X [0x%X]",
