...
public class DMX512SerialDataDecoder extends AsyncSerialDataDecoder
{
  ...
  {
    int sampleRate = this.data.getSampleRate();
    ...
    if ( ( time2 - time ) > this.configuration.getFrameSize( sampleRate ) )
    {
      ...
      if ( callback != null )
      {
        ...
        callback.onEvent( aChannelIdx, DMX512AnalyzerTask.EVENT_SBB, time, time2 );
        // The second part is the mark-after-break...
        callback.onEvent( aChannelIdx, DMX512AnalyzerTask.EVENT_MAB, time2, time3 );
      }
