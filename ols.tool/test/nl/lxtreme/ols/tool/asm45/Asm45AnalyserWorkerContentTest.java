...
import static nl.lxtreme.ols.tool.asm45.Asm45AnalyserTask.*;
import java.net.*;
import java.util.*;
 ...
 */
public class Asm45AnalyserWorkerContentTest extends TestCase
{
  // METHODS
   ...
   */
  public void testAsm45DecodingOk() throws Exception
  {
    final AnnotationCollector result = analyseDataFile( "asm45_test.ols", 22, 23, 25, 26, 27, 29, 30 );
   ...
   */
  private AnnotationCollector analyseDataFile( final String aResourceName, final int aLineSMCIndex,
      final int aLineSTMIndex, final int aLineEBGIndex, final int aLineBYTEIndex, final int aLineBLIndex,
      final int aLineWRTIndex, final int aLineSYNCIndex ) throws Exception
  {
    ...
    config.asMap().put( "smcIdx", Integer.valueOf( aLineSMCIndex ) );
    config.asMap().put( "stmIdx", Integer.valueOf( aLineSTMIndex ) );
    config.asMap().put( "ebgIdx", Integer.valueOf( aLineEBGIndex ) );
    config.asMap().put( "byteIdx", Integer.valueOf( aLineBYTEIndex ) );
    config.asMap().put( "blIdx", Integer.valueOf( aLineBLIndex ) );
    config.asMap().put( "wrtIdx", Integer.valueOf( aLineWRTIndex ) );
    config.asMap().put( "syncIdx", Integer.valueOf( aLineSYNCIndex ) );
    config.asMap().put( "reportInst", Boolean.TRUE );
    config.asMap().put( "reportData", Boolean.TRUE );
    config.asMap().put( "reportBusGrants", Boolean.TRUE );
   ...
   * @return
   */
  private void assertAddress( final AnnotationCollector aDataSet, final int... aExpectedAddress )
  {
    final Iterator<DataAnnotation> dataIter = aDataSet.dataAnnotations.iterator();

    int i = 0;
    while ( dataIter.hasNext() && ( i < aExpectedAddress.length ) )
    {
      final DataAnnotation data = dataIter.next();
      final int address = ( ( Integer )data.getProperties().get( KEY_ADDRESS ) ).intValue();

      assertEquals( "Address at index " + i + " not equal, ", aExpectedAddress[i], address );
      i++;
    }
   ...
   * @return
   */
  private void assertBlock( final AnnotationCollector aDataSet, final int... aExpectedBlock )
  {
    final Iterator<DataAnnotation> dataIter = aDataSet.dataAnnotations.iterator();
    ...
    {
      final DataAnnotation data = dataIter.next();
      final int block = ( ( Integer )data.getProperties().get( KEY_BLOCK ) ).intValue();

      assertEquals( "Block at index " + i + " not equal, ", aExpectedBlock[i], block );
      i++;
    }
   ...
   * @return
   */
  private void assertValue( final AnnotationCollector aDataSet, final int... aExpectedValue )
  {
    final Iterator<DataAnnotation> dataIter = aDataSet.dataAnnotations.iterator();
    ...
    {
      final DataAnnotation data = dataIter.next();
      final int value = ( ( Integer )data.getProperties().get( KEY_IDA ) ).intValue();

      assertEquals( "Value at index " + i + " not equal, ", aExpectedValue[i], value );
      i++;
    }
