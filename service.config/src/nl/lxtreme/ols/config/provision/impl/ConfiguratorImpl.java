...
public class ConfiguratorImpl implements Configurator
{
  ...
  protected void start( final Component component ) throws IOException
  {
    ...
    File[] configDirs = getConfigDirectories( context );
  ...
  private File[] getConfigDirectories( final BundleContext context ) throws IOException
  {
    String prop = context.getProperty( KEY_CONFIG_DIR );
    if ( prop == null || "".equals( prop.trim() ) )
    {
      throw new IOException( "No configuration directory set! Use property " + KEY_CONFIG_DIR + " for that!" );
    }
    String[] dirNames = prop.split( "\\s*,\\s*" );
    File[] result = new File[dirNames.length];
    for ( int i = 0; i < result.length; i++ )
    {
      result[i] = new File( dirNames[i] );
    }
    return result;
  }
