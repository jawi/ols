...
public final class SpringLayoutUtils
{
  ...
  public static final Container addSeparator( final Container aContainer, final String aText )
  {
    ...
    // than on other OSs...
    separator.setAlignmentY( isMacOS() ? 0.25f : Component.CENTER_ALIGNMENT );
  ...
  {
    makeEditorGrid( aContainer, aInitialX, aInitialY, 6, 6 );
  }
   ...
   */
  public static void makeEditorGrid( final Container aContainer, final int aInitialX, final int aInitialY,
      final int aXpad, final int aYpad )
  {
    if ( !( aContainer.getLayout() instanceof SpringLayout ) )
    {
      throw new IllegalArgumentException( "Container should have SpringLayout as layout manager!" );
    ...
    for ( int c = 0; c < columns; c++ )
    {
      ...
      Spring spacing = ( c == ( columns - 1 ) ) ? Spring.constant( 0 ) : Spring.constant( aXpad );
      x = Spring.sum( x, Spring.sum( width, spacing ) );
    }
    ...
    for ( int r = 0; r < rows; r++ )
    {
      ...
      Spring spacing = ( r == ( rows - 1 ) ) ? Spring.constant( 0 ) : Spring.constant( aYpad );
      y = Spring.sum( y, Spring.sum( height, spacing ) );
    }
