...
public final class ExportUtils
{
  ...
  public static class WriterOutputStream extends OutputStream
  {
     ...
     * @param aWriter
     */
    public WriterOutputStream( final Writer aWriter )
    {
      this.writer = aWriter;
    }
     ...
     * @param aEncoding
     */
    public WriterOutputStream( final Writer aWriter, final String aEncoding )
    {
      this.writer = aWriter;
     ...
     */
    @Override
    public void write( final byte[] aByteArray ) throws IOException
    {
      if ( this.encoding == null )
      {
        this.writer.write( new String( aByteArray ) );
      }
      else
      {
        this.writer.write( new String( aByteArray, this.encoding ) );
      }
    }
     ...
     */
    @Override
    public void write( final byte[] aByteArray, final int aOffset, final int aLength ) throws IOException
    {
      if ( this.encoding == null )
      {
        this.writer.write( new String( aByteArray, aOffset, aLength ) );
      }
      else
      {
        this.writer.write( new String( aByteArray, aOffset, aLength, this.encoding ) );
      }
    }
     ...
     */
    @Override
    public synchronized void write( final int aByte ) throws IOException
    {
      this.buf[0] = ( byte )aByte;
      write( this.buf );
    }
  }
