 ...
 */
package nl.lxtreme.ols.client.ui.signaldisplay.view.signals;
...
public class SignalView extends AbstractViewLayer implements IMeasurementListener, IMarkerChangeListener
{
    ...
    public void mouseMoved( final MouseEvent aEvent )
    {
      ...
      if ( !aEvent.isConsumed() )
      {
        ...
        if ( ( element != null ) && element.isDigitalSignal() )
        {
          ...
          Annotation annotation = helper.getAnnotation( channelIdx, timestamp );
          if ( annotation != null )
          {
            SignalView view = ( SignalView )aEvent.getSource();
            view.setToolTipText( helper.getDescription( annotation, true /* aIncludeTimingData */) );
  ...
  public void markerChanged( final String aPropertyName, final Marker aNewCursor )
  {
    ...
    int cursorPos = model.timestampToCoordinate( aNewCursor.getTimestamp() );
  ...
  public void markerMoved( final long aOldTimestamp, final long aNewTimestamp )
  {
    ...
    int cursorPos = model.timestampToCoordinate( aOldTimestamp );
    repaint( 0, cursorPos - 1, y, 2, height );

    cursorPos = model.timestampToCoordinate( aNewTimestamp );
    repaint( 0, cursorPos - 1, y, 2, height );
  }
