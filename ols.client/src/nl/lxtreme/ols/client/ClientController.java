 ...
 */
public final class ClientController implements EventHandler, ActionProvider, AnnotationListener, ApplicationCallback,
    ToolProgressListener
{
  ...
  final class DefaultToolContext implements ToolContext
  {
    ...
    public void clearAnnotations( final int... aChannelIdxs )
    {
      ...
      {
        ClientController.this.clearAnnotations( channelIdx );
      }
    }
    ...
    {
      ClientController.this.onAnnotation( aAnnotation );
    }
    ...
    {
      return ClientController.this.session;
    }
  ...
  private volatile DataAcquirer dataAcquirer;
   ...
   */
  public void addTool( final ToolInvoker aToolInvoker )
  {
  ...
  public void clearAnnotations()
  {
    ...
    {
      this.dockController.annotationOverview.clearAnnotations();
    }
  ...
  public void clearAnnotations( final int aChannelIdx )
  {
    ...
    if ( channel != null )
    {
      ...
      if ( this.dockController != null )
      {
        this.dockController.annotationOverview.clearAnnotations( aChannelIdx );
      }
  ...
  public void handleEvent( final Event aEvent )
  {
    ...
    {
      final AcquisitionData data = ( AcquisitionData )aEvent.getProperty( KEY_ACQUISITION_DATA );
    ...
    else if ( TOPIC_ACQUISITION_PROGRESS.equals( topic ) )
    {
      ...
      this.progressAccumulatingRunnable.add( percentage );
    }
    else if ( TOPIC_ACQUISITION_STATUS.equals( topic ) )
    {
      final String status = ( String )aEvent.getProperty( KEY_STATUS );
      final String device = ( String )aEvent.getProperty( KEY_DEVICE );
      ...
      else if ( STATUS_SUCCESS.equals( status ) )
      {
        ...
        setStatusOnEDT( "Acquisition from {2} finished at {0,date,medium} {0,time,medium}, and took {1}.", new Date(),
      ...
      else if ( STATUS_FAILED.equals( status ) )
      {
        ...
        setStatusOnEDT( "Acquisition from {1} failed! Reason: {0}", exception.getMessage(), device );
      }
      else if ( STATUS_CANCELLED.equals( status ) )
      {
        setStatusOnEDT( "Acquisition cancelled for {0}.", device );
      }
  ...
  public void invokeTool( final String aToolName, final Window aParent )
  {
    ...
    else
    {
      ...
      if ( tool.configure( aParent, context ) )
      {
        tool.invoke( context );
      }
    }
  ...
  {
    final int channelIdx = aAnnotation.getChannelIndex();
    getChannel( channelIdx ).addAnnotation( aAnnotation );

    // TODO
    if ( this.dockController != null )
    {
      this.dockController.annotationOverview.onAnnotation( aAnnotation );
    }
   ...
   */
  public void removeTool( final ToolInvoker aTool )
  {
  ...
  public void repeatCaptureData()
  {
    ...
    catch ( final IOException exception )
    {
      ...
      // Make sure to handle IO-interrupted exceptions properly!
      IOUtil.handleInterruptedException( exception );
    }
    finally
    {
      updateActionsOnEDT();
    ...
    SwingUtilities.invokeLater( new Runnable()
    {
      ...
      public void run()
      {
        ...
        System.setProperty( JErrorDialog.PROPERTY_REPORT_INCIDENT_EMAIL_ADDRESS,
            hostProperties.getReportIncidentAddress() );
        ...
        ClientController.this.dockController = new DockController( dataStorage, getSignalDiagramController() );
  ...
  final void updateActionsOnEDT()
  {
    SwingComponentUtils.invokeOnEDT( new Runnable()
    {
      ...
      public void run()
      {
        ...
        final boolean deviceCapturing = deviceControllerSet
            && ClientController.this.dataAcquirer.isAcquisitionInProgress( device );
   ...
   * {@inheritDoc}
   */
  private ToolInvoker getTool( final String aName ) throws IllegalArgumentException
  {
