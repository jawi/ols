 ...
 */
public class AnnotationOverview extends AbstractViewLayer implements IToolWindow, AnnotationListener,
    IDataModelChangeListener
{
  ...
  private static class AnnotationContainer extends AbstractTableModel
  {
     ...
     */
    public AnnotationContainer( final DataSet aDataSet )
    {
      ...
      AcquisitionData data = aDataSet.getCapturedData();

      this.sampleRate = data.getSampleRate();
      this.triggerPos = data.hasTriggerData() ? data.getTriggerPosition() : 0L;
    }
    ...
    private String getChannelName( final Annotation aAnnotation )
    {
      ...
      Channel channel = this.dataSet.getChannel( channelIdx );
      if ( channel == null )
      {
        // TODO verify!!!
  ...
  {
    this.container.clear( aChannelIdx );
  }
  ...
  {
    AcquisitionData capturedData = aDataSet.getCapturedData();
    if ( capturedData != null )
    {
      this.container = new AnnotationContainer( aDataSet );
  ...
  {
    this.container.add( aAnnotation );
  }
