...
public class Asm45AnalyserWorker extends BaseAsyncToolWorker<Asm45DataSet>
{
  // CONSTANTS
   ...
   * @param aContext
   */
  public Asm45AnalyserWorker( final DataContainer aData, final ToolContext aContext )
  {
   ...
   */
  @Override
  protected Asm45DataSet doInBackground() throws Exception
  {
    ...
    final Asm45DataSet asm45DataSet = new Asm45DataSet( startOfDecode, endOfDecode, this );
    ...
    for ( ; idx < ( asm45DataSet.getEndOfDecode() - 1 ); idx++ )
    {
      ...
      if ( ( ( status & this.lineSMCmask ) == 0 ) && ( ( control & this.lineSMCmask ) != 0 ) )
      {
        ...
        if ( ( ( status & this.lineEBGmask ) != 0 ) && ( ( control & this.lineEBGmask ) == 0 ) )
        {
          ...
          if ( ( status & this.lineWRTmask ) != 0 )
          {
            if ( address < 32 )
            {
              event = Asm45AnalyserWorker.registers[address] + String.format( "&rarr;$%04x", ida );
            }
            else
            {
          ...
          else
          {
            if ( address < 32 )
            {
              event = Asm45AnalyserWorker.registers[address] + String.format( "&larr;$%04x", ida );
            }
            else
            {
      ...
      setProgress( getPercentage( idx, asm45DataSet.getStartOfDecode(), asm45DataSet.getEndOfDecode() ) );
    }
  ...
  protected String word2asm( final int address, final int opcode )
  {
    ...
    switch ( op.getMode() )
    {
      ...
      case 7:
        ...
        operand = opcode & 0x7;
        ret_string += String.format( " %s", Asm45AnalyserWorker.registers[operand] );
        if ( ( opcode & 0x0080 ) != 0 )
        {
          ret_string += ",D";
        }
