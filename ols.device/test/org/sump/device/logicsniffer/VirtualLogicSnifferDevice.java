...
public class VirtualLogicSnifferDevice extends LogicSnifferAcquisitionTask
{
  ...
  /**
   * An {@link DeviceProgressListener} that does nothing.
   */
  static final class NullAcquisitionProgressListener implements DeviceProgressListener
  {
    /**
     * {@inheritDoc}
     */
      ...
      final long aExpectedLength ) throws IOException
  {
    ...
    assertArrayEquals( "Sample values not as expected?!", expectedValues, actualValues );
    ...
    assertArrayEquals( "Timestamps not as expected?!", expectedTimestamps, actualTimestamps );
  ...
  protected StreamConnection getStreamConnection()
  {
    final StreamConnection connection = new StreamConnection()
    {
      ...
      {
        IOUtil.closeResource( this.is );
        IOUtil.closeResource( this.os );
      }
  ...
  private void assertArrayEquals( final String aMessage, final int[] expected, final int[] input )
  {
    assertEquals( "Length mismatch!", expected.length, input.length );
    for ( int i = 0; i < expected.length; i++ )
    {
      assertEquals( "Element @ " + i + " " + aMessage, expected[i], input[i] );
    }
  }

  private void assertArrayEquals( final String aMessage, final long[] expected, final long[] input )
  {
    assertEquals( "Length mismatch!", expected.length, input.length );
    for ( int i = 0; i < expected.length; i++ )
    {
