 ...
 * <p>
 * The following methods cause the transition to the Connected state when the
 ...
 * </UL>
 * <P>
 * After an output stream has been opened by the <code>openOutputStream</code>
 * or <code>openDataOutputStream</code> methods, attempts to change the request
 * parameters via <code>setRequestMethod</code> or the
 * <code>setRequestProperty</code> are ignored. Once the request parameters have
 * been sent, these methods will throw an <code>IOException</code>. When an
 * output stream is closed via the <code>OutputStream.close</code> or
 * <code>DataOutputStream.close</code> methods, the connection enters the
 ...
 * <code>close</code> method and the closing all of the streams that were opened
 * from the connection.
 * <P>
 * <STRONG>Example using StreamConnection</STRONG>
 * <p>
 * Simple read of a URL using <code>StreamConnection</code>. No HTTP specific
 * behavior is needed or used. (<strong>Note:</strong> this example ignores all
 * HTTP response headers and the HTTP response code. Since a proxy or server may
 * have sent an error response page, an application can not distinquish which
 * data is retreived in the <code>InputStream</code>.)
 * <p>
 * <code>Connector.open</code> is used to open URL and a
 * <code>StreamConnection</code> is returned. From the
 * <code>StreamConnection</code> the <code>InputStream</code> is opened. It is
 * used to read every character until end of file (-1). If an exception is
 ...
 * <p>
 * Simple read of a URL using <code>ContentConnection</code>. No HTTP specific
 * behavior is needed or used.
 * <p>
 * <code>Connector.open</code> is used to open url and a
 * <code>ContentConnection</code> is returned. The
 * <code>ContentConnection</code> may be able to provide the length. If the
 * length is available, it is used to read the data in bulk. From the
 * <code>ContentConnection</code> the <code>InputStream</code> is opened. It is
 * used to read every character until end of file (-1). If an exception is
 * thrown the connection and stream are closed.
 * <p>
 * 
 ...
 * <p>
 * <code>Connector.open</code> is used to open url and a
 * <code>HttpConnection</code> is returned. The HTTP headers are read and
 * processed. If the length is available, it is used to read the data in bulk.
 * From the <code>HttpConnection</code> the <code>InputStream</code> is opened.
 * It is used to read every character until end of file (-1). If an exception is
 * thrown the connection and stream are closed.
 * <p>
 * 
 * <pre>
 ...
 * <p>
 * Post a request with some headers and content to the server and process the
 * headers and content.
 * <p>
 * <code>Connector.open</code> is used to open url and a
 * <code>HttpConnection</code> is returned. The request method is set to POST
 ...
 * read the data in bulk. From the <code>HttpConnection</code> the
 * <code>InputStream</code> is opened. It is used to read every character until
 * end of file (-1). If an exception is thrown the connection and stream is
 * closed.
 * <p>
 * 
 * <pre>
 ...
 * <p>
 * Please note the following: The <code>Connector</code> class defines the
 ...
 * </UL>
 * Please be aware that using these methods implies certain restrictions. You
 * will not get a reference to the actual connection, but rather just references
 * to the input or output stream of the connection. Not having a reference to
 * the connection means that you will not be able to manipulate or query the
 * connection directly. This in turn means that you will not be able to call any
...
public interface HttpConnection extends ContentConnection
{
   ...
   * (<strong>Note:</strong> the name of this status code reflects the earlier
   * publication of RFC2068, which was changed in RFC2616 from
   * "moved temporalily" to "found". The semantics were not changed. The
   * <code>Location</code> header indicates where the application should resend
   * the request.)
   */
   ...
   * timely response from the upstream server specified by the URI or some other
   ...
   * 
   * @return the expiration date of the resource that this URL references, or 0
   *         if not known. The value is the number of milliseconds since January
   *         1, 1970 GMT.
   * @exception IOException
   *              if an error occurred connecting to the server.
   */
   ...
   * 
   * @return the value of the nth header field or <code>null</code> if the array
   *         index is out of range. An empty String is returned if the field
   ...
   * @param name
   *          of a header field.
   * @return the value of the named header field, or <code>null</code> if there
   ...
   * field.
   * <p>
   * This form of <code>getHeaderField</code> exists because some connection
   * types (e.g., <code>http-ng</code>) have pre-parsed headers. Classes for
   * that connection type can override this method and short-circuit the
   * parsing.
   * 
   * @param name
   ...
   *         <code>def</code> argument is returned if the field is missing or
   *         malformed.
   * @exception IOException
   *              if an error occurred connecting to the server.
   */
   ...
   *         <code>def</code> value is returned if the field is missing or
   *         malformed.
   * @exception IOException
   *              if an error occurred connecting to the server.
   */
   ...
   * 
   * @return the key of the nth header field or <code>null</code> if the array
  ...
  /**
   * Returns the host information of the URL of this <code>HttpConnection</code>
   * . e.g. host name or IPv4 address
   * 
   * @return the host information of the URL of this <code>HttpConnection</code>
   *         .
   */
   ...
   *         <code>HttpConnection</code>. The default HTTP port number (80) is
   *         returned if there was no port number in the string passed to
   ...
   * RFC2396 specifies the optional fragment identifier as the the text after
   ...
   * identifier is dependent on the media type[RFC2046] of the retrieved
   * information.
   * 
   ...
   * 
   * @param key
   *          the keyword by which the request property is known (e.g.,
   *          "accept").
   ...
   * </PRE>
   * 
   * and extracts the ints 200 and 401 respectively. from the response (i.e.,
   ...
   * none could be discerned from the responses (the result was not valid HTTP).
   * 
   * @exception IOException
   *              if an error occurred connecting to the server.
   ...
   *          the HTTP method
   * @exception IOException
   *              if the method cannot be reset or if the requested method isn't
   *              valid for HTTP.
   ...
   * <p>
   * <strong>Note:</strong> HTTP requires all request properties which can
   * legally have multiple instances with the same key to use a comma-separated
   * list syntax which enables multiple properties to be appended into a single
   * property.
   * 
   * @param key
   ...
   * @exception IOException
   *              is thrown if the connection is in the connected state.
